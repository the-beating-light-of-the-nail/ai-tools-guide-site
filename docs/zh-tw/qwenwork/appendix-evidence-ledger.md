...
這一頁是這本書的「證據底賬」。你在別處讀到的每一條主張——比如「自動化必須能停下來才配進企業執行」「階段門不能用日曆時間代替」——都能在這裡查到它憑什麼這麼說：來源是什麼、核驗到什麼程度、適用範圍到哪、侷限在哪。

你什麼時候會來翻它？三種情況最典型：有人質疑你引用的結論時；你要把它搬進自己公司的材料、需要先確認邊界時；或者你想知道某條建議到底有沒有實據，還是只是社群判斷。

你不用從頭讀到尾。你要查某一條，直接在頁面裡搜主張編號（形如 `claim-xxx-01`），或者看它「正文位置」那一列，跳回原章節。

## 怎麼讀這張表

每張卡片是一張兩列表格，你重點看這幾行：

- **來源型別**：標 `community-framework` 的是社群框架判斷，不是行業統計事實。你引用時要連同這個性質一起說。
- **核驗狀態**：`editor-reviewed` 表示已經過編輯複核；沒標的不要當結論用。
- **適用範圍** 與 **侷限**：這兩行決定你能不能把它搬進自己的場景。你跳過這兩行直接摘結論，最容易出問題。
- **正文位置**：點進去能跳回提出這條主張的章節。你要看上下文時走這裡。
...
現金 ROI 與可釋放產能必須分開報告；質量和風險也單獨報告，只有形成可核驗的實際損失或收益時才進入現金口徑。
...
| 統計口徑 | 同口徑任務時長、有效任務量、現金憑證及質量風險記錄 |
| 適用範圍 | 企業 AI 試點價值報告、預算復盤和規模化決策 |
| 侷限 | 可釋放產能不保證形成支出減少或增量毛利；質量和風險金額只有在實際發生且可核驗時才進入現金賬。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 財務與價值複核者 |
...
| 適用範圍 | 企業 AI 場景試點、團隊釋出、規模化推廣和事件復盤 |
...
| 侷限 | 欄位完整只能提高可複核性，不能保證來源真實、口徑正確或結論成立。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 研究方法負責人 |

## claim-expert-kit-release-01

專家套件只能組合已經通過崗位階段門的工作流，不能用套件名稱替代組成資產的測試、權限和責任記錄。
...
G2 只有在工作流正式釋出、邊界明確且取得共同批准的持續授權後，才可從逐批事前確認轉為批准範圍內執行和抽檢；G3 每次仍須事前確認。
...
| 侷限 | 持續授權只適用於登記的工作流版本、執行主體、執行環境、批准用途和業務邊界；權限、欄位、接收物件、錯誤率、工作流邏輯或規則、版本、模型、連接器、執行主體或執行環境任一變化時均立即失效，並要求重驗和共同批准。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 資料與安全治理負責人 |

## claim-human-responsibility-01

業務負責人始終對最終業務結果負責，不能把責任轉移給 AI 或流程維護者；G3 執行者與動作批准人不得為同一人。
...
| 適用範圍 | 企業 AI 工作流的設計、執行、複核、批准、事件處置和結果問責 |
| 侷限 | 角色可由小團隊成員兼任，但不得破壞 G3 執行與批准分離；具體法定責任仍以適用法律和組織制度為準。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 企業治理負責人 |

## claim-office-delivery-acceptance-01

檔案、資料、彙報和網頁任務都按輸入、處理、產物、驗收、風險、人工確認設計，不能把檔案成功生成當成交付通過。
...
| 侷限 | R11 未暴露可把單個示例產物穩定繫結到具體候選案例的案例級記錄；該邊界不評價示例產物本身的質量。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 案例證據複核者 |
...
Skill 封裝可重複方法和工具步驟；它不自動取得資料權限，也不替代專業驗收。
...
| 侷限 | 官方頁面用於確認 Skill 的產品語境；權限和專業驗收邊界是本書的治理要求。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 工作流資產負責人 |
...
| 侷限 | 職責分離是本書針對 G3 高後果動作的社群治理建議，不是產品官方授權或行業統計；小團隊可兼任其他角色，但不得由同一人執行並批准 G3 動作。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 編輯複核者 |

## claim-summary-roles-01

責任鏈採用業務負責人、流程維護者、資料/安全責任人、使用者、內容複核者、動作批准人六類角色，業務負責人始終對最終業務結果負責。
...
| 統計口徑 | 不適用：社群判斷 |
| 適用範圍 | 企業 AI 工作流的責任分配、上線批准和結果追責 |
| 侷限 | 六類角色是本書的社群責任模型，不是法定崗位編制、產品官方角色或行業統計；小團隊可兼任，但各項責任仍須明確，且 G3 執行者與動作批准人必須分離。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 編輯複核者 |
...
| 侷限 | 階段門是本書的社群治理判斷，不代表產品官方承諾或行業統計；通過階段門只說明具備升級條件，不授予資料訪問、寫入、傳送或釋出權限。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 編輯複核者 |

## claim-summary-value-reporting-01

使用量不等於價值；現金 ROI、可釋放產能、質量和風險應分開報告，節省工時不能直接寫成現金收益。
...
0–30 天只驗證個人模板候選；階段最低批准人為使用者與業務負責人，最終批准人取該集合與全部適用硬門批准人的並集；以交付物、驗收和異常記錄為證據，任一適用硬門失敗或驗收未通過即停止升級。
...
31–60 天只驗證團隊工作流釋出；階段最低批准人為業務負責人、流程維護者與資料/安全責任人，最終批准人取該集合與全部適用硬門批准人的並集；以工作流版本、團隊驗收、批准和異常記錄為證據，任一適用硬門失敗即停止釋出，其他釋出門未通過則最佳化後再測或停止。
...
| 侷限 | 31–60 天是團隊釋出驗證的社群規劃標籤，不是產品官方釋出週期或行業統計；版本、邊界或權限變化時即使已到第 60 天也應退回重驗，不能自動釋出。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 編輯複核者 |

## claim-summary-window-90-01

61–90 天只驗證規模化推廣；階段最低批准人為業務負責人與資料/安全責任人，最終批准人取該集合與全部適用硬門批准人的並集；以覆蓋業務週期的執行、質量、效率和權限證據作決定，任一適用硬門失敗、登記目標未達到或權限問題未解決即停止推廣。
...
| 侷限 | 61–90 天是規模化驗證的社群規劃標籤，不是產品官方承諾或行業統計；未覆蓋業務週期、登記目標未達到或權限問題未閉環時，不得以到期為由推廣。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 編輯複核者 |
...
| 統計口徑 | 不適用：任務輸入規範 |
| 適用範圍 | 檔案、資料、研究、彙報和協同類 AI 任務 |
| 侷限 | 五個欄位規定最低任務合同，不替代各專業領域的法規、質量標準或審批要求。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 方法規範負責人 |
...
| 適用範圍 | 企業 AI 試點運營看板、價值復盤和擴大決策 |
| 侷限 | 使用指標仍可用於判斷採用和負載，但必須與同口徑效率、質量、風險和現金證據共同解釋。 |
| 核驗日期 | 2026-08-01 |
| 責任角色 | 價值度量負責人 |

## claim-workflow-card-fields-01

團隊工作流卡唯一規範由 Owner（業務負責人）、流程維護者、版本、適用範圍、輸入輸出、權限、測試樣本、質量門、變更記錄、失敗案例、回退方式、執行日誌和復盤節奏十三個欄位組成。
...
欄位、權限、模型、連接器或接收物件發生變化時，工作流必須停止沿用舊驗證結論，按受影響範圍重新測試、複核和批准。
...
| 統計口徑 | 不適用：版本變化後的重驗和批准規則 |
| 適用範圍 | 包含欄位、權限、模型、連接器或接收物件的團隊工作流 |
...
不全是。你要看每張卡片的「來源型別」：標 `community-framework` 的是本書的社群判斷，「侷限」那一列通常也會寫明「不代表已經得到行業統計證明的普遍事實」。你對外引用時要連同這個邊界一起說。

**我想核實某一條，從哪入手？**

你先看卡片的「正文位置」，跳回提出它的章節讀上下文；再看「來源定位」，確認原始出處。你找不到來源定位的，就按未核驗處理。

**核驗狀態有幾檔，分別什麼意思？**

本頁出現的 `editor-reviewed` 表示已經過編輯複核。你看到沒標核驗狀態的條目，不要當成結論引用。

**可以整段複製到我公司的材料裡嗎？**

內容公開，你可以引用，但你得帶上適用範圍和侷限。你只摘結論不摘邊界，是最容易出事的用法。

**臺賬多久更新一次？**

本頁由結構化資料自動生成，源資料在上游專案維護。你看到的「核驗日期」是每條主張各自的複核時間，不是整頁的更新時間。

**為什麼有的卡片有統計口徑，有的寫「不適用」？**

只有量化結論才需要統計口徑。你看到「不適用：社群判斷」的，說明這條是判斷而非統計，別拿它當資料用。

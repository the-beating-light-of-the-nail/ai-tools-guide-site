...
这一页是这本书的「证据底账」。你在别处读到的每一条主张——比如「自动化必须能停下来才配进企业运行」「阶段门不能用日历时间代替」——都能在这里查到它凭什么这么说：来源是什么、核验到什么程度、适用范围到哪、局限在哪。

你什么时候会来翻它？三种情况最典型：有人质疑你引用的结论时；你要把它搬进自己公司的材料、需要先确认边界时；或者你想知道某条建议到底有没有实据，还是只是社区判断。

你不用从头读到尾。你要查某一条，直接在页面里搜主张编号（形如 `claim-xxx-01`），或者看它「正文位置」那一列，跳回原章节。

## 怎么读这张表

每张卡片是一张两列表格，你重点看这几行：

- **来源类型**：标 `community-framework` 的是社区框架判断，不是行业统计事实。你引用时要连同这个性质一起说。
- **核验状态**：`editor-reviewed` 表示已经过编辑复核；没标的不要当结论用。
- **适用范围** 与 **局限**：这两行决定你能不能把它搬进自己的场景。你跳过这两行直接摘结论，最容易出问题。
- **正文位置**：点进去能跳回提出这条主张的章节。你要看上下文时走这里。
...
不全是。你要看每张卡片的「来源类型」：标 `community-framework` 的是本书的社区判断，「局限」那一列通常也会写明「不代表已经得到行业统计证明的普遍事实」。你对外引用时要连同这个边界一起说。

**我想核实某一条，从哪入手？**

你先看卡片的「正文位置」，跳回提出它的章节读上下文；再看「来源定位」，确认原始出处。你找不到来源定位的，就按未核验处理。

**核验状态有几档，分别什么意思？**

本页出现的 `editor-reviewed` 表示已经过编辑复核。你看到没标核验状态的条目，不要当成结论引用。

**可以整段复制到我公司的材料里吗？**

内容公开，你可以引用，但你得带上适用范围和局限。你只摘结论不摘边界，是最容易出事的用法。

**台账多久更新一次？**

本页由结构化数据自动生成，源数据在上游项目维护。你看到的「核验日期」是每条主张各自的复核时间，不是整页的更新时间。

**为什么有的卡片有统计口径，有的写「不适用」？**

只有量化结论才需要统计口径。你看到「不适用：社区判断」的，说明这条是判断而非统计，别拿它当数据用。

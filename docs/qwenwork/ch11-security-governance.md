...
这一章帮你分清：同样一份会议纪要，读取、存草稿、写进内部台账、发给客户，风险根本不是一档。你带团队上 AI 办公时，最容易犯的错是按「读还是写」一刀切——其实读候选人信息也碰隐私，生成法律判断存草稿也可能影响高后果决定。本章用 G0–G3 标动作风险等级，再叠加数据敏感度和专业后果，三者取最严。你读完就知道，G2 写内部台账默认每批确认，只有正式发布且拿到持续授权才能抽检；G3 对外发送每次都要确认，且执行人和批准人不能是同一个人。你以后授权少踩坑，也清楚出事时谁该停、谁该接。你上手前先想动作和后果，你拿不准就走最严档，你分工写清才追得到人，你别跳级。
...
白话解释：你记牢三条——G0–G3 只是动作风险等级，不是「能不能执行」的标签；机密数据或高后果判断会把它往上抬，按最严的来。G2 默认每批确认，只有正式发布且共同批了持续授权才能抽检；G3 每次都要确认，且干活的人和签字的人不能是一个人。
...
白话解释：你别只用「读/写」分风险——读候选人信息也碰隐私，存一份法律判断草稿也可能左右人事决定。你把动作、数据、后果拆开看，才知道谁担结果、怎么回退、控制下限设多严；等级只是控制强度的一个维度，不是成熟度。
...
白话解释：你判断 G1「可逆」不能嘴上说说——得真能找到对象、找回原件、验证恢复。你覆盖原件就不算 G1；G2 的确认必须让批准人看到这批的对象、字段和影响，不能用一个通用弹窗或「上次同意过」糊弄。
...
白话解释：你分数据敏感度时，机密和个人信息控制不低于 G2，受监管或组织秘密直接按 G3。你别因为「只是内部数据」就放松；你按数据定控制，不按动作顺手与否。你列清单时把最敏感那类单独标出，你授权范围就缩得最小。
...
白话解释：你读个人信息这个动作本身还是 G0，但因为数据敏感，控制得按不低于 G2 来——授权、访问记录、复核一个都不能少。你升级的是控制强度，不是把动作改名；标签不变，要求变严。
...
白话解释：你小团队人手紧可以一人兼几角，但「干 G3 活的人」和「签 G3 批准的人」必须分开，不能自己批自己。你这些分工再细，也免不掉业务负责人对最终结果兜底。
...
白话解释：你拿到 G2 持续授权后，它只绑死在登记的那一版工作流、那个账号、那个环境、那个用途上。你权限、字段、接收对象或错误率一动，授权立刻作废；版本或模型一变也要先停用、回逐批确认，重验重批才能再开。到期不自动续。
...
白话解释：你 G3 动作没有「持续授权」这回事，每次都得重新确认。你展示的必须是这次的接收人、动作、关键字段和不可逆后果；对象一变、过期了、要重试了，全都要重新亮出来給批准人看，抽检顶替不了事前批准。
...
白话解释：你同一条「客户拜访跟进」流程里，读 CRM 是 G0、存草稿是 G1、写内部状态是 G2、发给客户是 G3。你 G2 哪怕拿了持续授权，最后那一下对外发邮件仍要逐次确认；出事后靠留痕才能定位是权限、内容还是确认环节出的问题。
...
白话解释：你补治理卡要逐动作标等级，不能整条流程给一个平均等级糊弄。你 G1 要真演练一次恢复，G2 要逐批预览确认、持续授权就填全字段共同批，G3 要逐次确认且不准抽检豁免。你少一个必要控制，就缩范围或退回人工。
...
白话解释：你拿等级当不成「这流程有用」的证明，过阶段门也不自动给权限。你最好把第 10 章阶段门和自动化接管链一起用，让流程既过了质量价值验证，又能在权限、输入或错误率一变时就停、就接、就恢复。

## 术语翻成人话

| 术语 | 人话 |
|---|---|
| G0–G3 | 动作风险四档：只读、存草稿、写内部状态、对外高后果；你按动作定档，不是按成熟度；你别跳档替自己松绑 |
| 数据敏感度 | 数据分公开/内部/机密/受监管，你越敏感控制越严，机密至少按 G2；你授权范围缩到最小 |
| 专业后果 | 输出会不会影响法律、财务、人事等决定，你拿不准就按更高档控制；你让有资质的人签字 |
| 持续授权 | G2 正式发布后批的「范围内抽检」许可，你绑死版本和账号，一变就作废 |
| 动作批准人 | 干 G2/G3 前签字的人，你干 G3 时这人不能和干活的是同一个；你签字即担责 |
| 六类角色 | 业务负责人、流程维护者、数据/安全、使用者、内容复核者、动作批准人，你分工清责，出事才追得到人 |
| 恢复路径 | 真能找回原件、验证恢复的通道；你只有「能删除」口号不算 G1，你得真演练过 |

## 常见问题

**你判断一份会议纪要该归哪档，只看「读还是写」够吗？**
不够。你读候选人信息也碰隐私，存一份法律判断草稿也可能影响人事决定。你要把动作、数据敏感度、专业后果拆开，三者取最严的控制。你别只按动作定档，你拆三层才稳。

**你给 G2 工作流申请了持续授权，是不是以后不用每批确认了？**
只有正式发布、边界可枚举、你和业务负责人共同批准，且绑死版本账号的，才能在范围内抽检。你权限、字段、接收对象或错误率一动，授权立刻作废，得先回逐批确认。你变了就作废，不是续期，你旧授权不能挪去别处。

**你让同一个人写 G3 对外发送又签批准，行不行？**
不行。G3 执行者和动作批准人必须分离，不能自己批自己。业务负责人对最终业务结果负责，但签字环节这道隔离不能省。你签字环节的分隔是硬要求，你隔离省了就追不到人。

**你 G3 动作拿到了一次批准，之后能靠抽检代替再确认吗？**
不能。G3 没有持续授权豁免，每次执行前都要重新展示接收人、动作、关键字段和不可逆后果并获批。你对象一变、过期了或要重试，全都要重新确认，抽检顶替不了。你每次都得重新亮给批准人，你重试也要重批。

**你小团队人手紧，能不能一人兼多个治理角色？**
可以兼任，但 G3 执行者和批准人必须分开，业务负责人的最终责任也免不掉。你分工再省，这道人和责任的边界不能模糊。你省角色可以，省隔离不行，你责任边界不能糊。

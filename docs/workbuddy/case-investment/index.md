...
# 怎么把股票研究变成日常？八条提示词加 stock-advisor 流水线

你打开炒股软件，满屏红绿，财报读不完、行业理不清、多空吵不停。投资这件事又密又依赖判断。但整理碎片信息、拆解复杂材料、把思考过程摊开——恰好是 AI 擅长的。这一篇讲在一次完整的股票研究里，AI 能替你干掉哪些低质量的重复劳动，把精力还给判断本身。你不用样样自己来，先把 AI 摆到研究助理的位置上。

> **风险提示：本章所有提示词、Skill 与案例均以“辅助研究”为目的，不构成任何投资建议。股市有风险，投资需谨慎。**

## 先想清楚：AI 在投资里该干什么

多数人对「AI 炒股」的想象是让它预测涨跌。但从真实的高频用法看，你值得用的价值集中在四类：
...
这四类都不是「预测涨跌」，而是**减少低质量思考的时间**。AI 在投资里最合理的位置，是一个不知疲倦、不带情绪、随叫随到的研究助理——它负责把事实底座打牢，把判断留给你。你动手前，和[办公三件套](/workbuddy/case-office/)一样先用五个问题定标：
...
你搭配思路：**日常盯盘和批量选股用 `a-share-analyst` 与 `financial-expert`；要对一只票下深功夫、出完整报告，用 `stock-advisor`；需要跳出单一视角、逼自己看反面时，叫上 `peers-advisory-group`。** 你挑 Skill 时记一条：别指望一个 Skill 包揽全流程，按需组合才稳。

## 一套可复用的研究提示词链路

你按「最简单 → 相对复杂」排列，覆盖从「查资料」到「下判断」的完整链路。你先用前三条建立事实底座，需要深挖时再往后走。你用法很简单：把 `【】` 里的占位换成你的标的，粘贴运行。

### Prompt 1｜给公司建一个“事实底座”

很多错误判断从认错业务就开始——你以为它靠 A 赚钱，结果利润主要来自 B。你这一步压缩「搞清楚事实」的时间成本。
...
你选的往往不是公司，而是行业。你让 AI 做行业的「第一性梳理」，但行业拐点、价格见底这类问题别指望它给答案。
...
从「看公司」到「看生意」的关键一步。你让 AI 帮你看清楚混杂型公司（主业 A、利润却来自 B）的钱到底怎么赚。
...
中间四步各解决一个具体问题，你照骨架填就行：明确要求 + 交叉验证 + 不给结论：
...
你把前七步的逻辑压进同一个框架的「投资者尽职调查报告」，强制区分事实与判断、强制交叉验证、强制推演空方逻辑与黑天鹅——用来对抗人最容易犯的**确认偏误**。你各家 AI 的 DeepResearch 模式里都很好用。骨架：
...
八条提示词单独看都好用，但你真要完整研究一只票：要手动串、换标的重来、数据靠眼睛、决策容易自嗨、交付靠手工。`stock-advisor` 把这条链路**从「一堆提示词」变成「按一次就跑完的流水线」**：
...
这里藏着 Skill 创作最值得你学的一点：**复用而非重写**。技术指标脚本复用 `a-share-analyst`，决策讨论复用 `peers-advisory-group`，排版复用 `magazine-layout`——它自己新写的只有少数几块。你做一个复杂 Skill，先把已有能力当积木，缺哪块补哪块，再用一条主线编排起来，这才是它最该被你记住的地方。

它还有两个「产品化」小设计：首次使用建档（问你风险偏好、投资周期、关注行业、仓位上限，存进记忆）；两种入口同一条流水线（上传截图走图像识别 + 数据验证，直接给代码走纯数据驱动）。

一句话：**把「一次严肃的股票研究」从半天的手工活，压缩成一次对话。** 你从「搬运和拼接」变成「拍板和质疑」。

## 实战：用 `stock-advisor` 跑一遍天孚通信（300394）

标的是天孚通信（300394），光通信 / CPO 板块。你实战跑一遍，整个过程三步递进：先看图、再看财报，再开一场私董会。
...
WorkBuddy 先从图里识别出这是天孚通信，当前股价约 368.70 元，然后给你结构化速读：MA5 > MA10 > MA20 标准多头排列仍在主升浪；但当日一根长上影线（最高冲 376.10 回落到 368.70）、MACD 红柱开始缩短、乖离率偏大；支撑看 MA5（347）/ MA10（319），压力看当日高点 376。这一步它没有猜涨跌，而是把「图里能读到的事实」结构化了。
...
- 比尔·盖茨：商业模式和行业格局角度
- 乔布斯：产品力和用户体验角度
讨论要求：每位先各自发表独立观点；然后交叉质询；再由每人一句话给出
...
主持人综合出**分投资者类型的执行方案**，并把决策挂到未来验证节点上：
...
对话结束后你可让它把整场分析生成杂志风格报告（本地存 PDF 或上传飞书）。回头看：`stock-advisor` 把八条散装提示词变成三轮对话就跑完的完整研究——**看图 → 看财报 → 开私董会 → 出报告**，而全程它没有替你做「买还是不卖」那个最关键的决定。

你用这套链路，记住一条：AI 帮你把事实摆清楚，不是替你下注。你把标准说清、把数字交叉验证、把关键决定自己拍板——研究才算真用顺了。你别把 AI 的报告当圣旨，也别跳过它整理好的事实底座。

## 常见错误与使用边界

你用 AI 做投资，最容易踩的坑都在下表里：
...
**AI 能替我决定买还是卖吗？**
不能，也不该。它不掌握实时全量信息，更不为你的钱负责。你只用它做事实梳理和多空推演，买卖自己拍板——这一步它全程没替你做。

**截图识别的数字能直接信吗？**
别全信。图像识别会看错，财报口径也会变。你关键数字一律交叉验证，别把截图当唯一真相。

**行业拐点、价格见底这种问题它能答吗？**
答不了。这类依赖前瞻信息和经验，AI 给不出。你让它梳理「该盯哪些领先指标」，拐点自己盯。

**stock-advisor 和普通提示词差在哪？**
你把八条散装提示词变成按一次就跑完的流水线：看图、看财报、开私董会、出报告三轮对话搞定。它复用已有 Skill 当积木，你不用手动串。

**私董会是不是走过场？**
不是。四位幕僚带着数据交叉辩论，系统还会联网更新到全网最新事实，再按投资者类型给你分档执行方案，并把决策挂到未来验证节点上——逼你看反面，对抗确认偏误。

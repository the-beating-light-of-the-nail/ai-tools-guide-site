...
首次使用：你通过官方入口打开 WorkBuddy 小程序并登录，查看当前处于本机还是云端模式；本机模式下确认目标电脑在线且连接正确。
...
1. 你打开 WorkBuddy，在左侧「助理」栏点齿轮，进入「助理设置」；
...
3. 你等待绑定二维码生成，用手机微信扫码；

![手机微信扫码绑定](/workbuddy/08-im-assistant/assets/004_微信助理-扫码绑定_TElmbNPG5o.png)

4. 卡片显示「已绑定」后，你先发一条只读测试指令；

![卡片显示「已绑定」，发只读测试指令](/workbuddy/08-im-assistant/assets/005_微信助理-已绑定_ZQtrb6jCmo.png)

5. 你要换微信账号时，先解绑当前账号，再重新扫码。

> 二维码有时效限制。停留在「绑定中」、二维码过期或扫码失败时，你关闭配置窗口后重新进入，必要时重启 WorkBuddy 并重新生成二维码。

## 接入飞书

你走飞书的话，步骤比微信多些，你照着走：

1. 你走 WorkBuddy → 设置 → 助理设置 → 选择飞书；
...
8. 你创建版本并发布应用，然后在飞书内向机器人发一条只读测试任务。
...
2. 你为应用添加机器人能力，填机器人名称、描述和头像并确认发布；
...
4. 你获取应用凭证，填回 WorkBuddy。优先在测试组织或测试群完成验证。
...
**微信助理绑定失败、二维码过期怎么办？**
二维码有时效。你停留在「绑定中」、过期或扫码失败时，关掉配置窗口重新进，必要时重启 WorkBuddy 重新生成二维码。绑定后先发一条只读测试指令，确认它只读取不乱动。

**飞书、钉钉接入为什么比微信麻烦？**
微信是扫码绑定，几步就好。飞书和钉钉要在开放平台建应用、配权限和回调，步骤多，但换来的是公司账号体系下的稳定接入。你按页面引导一步步填，优先在测试群验证。

**IM 助理能直接替我发消息吗？**
能，但要看你授权了什么。它只在授权范围内动作——建会议、读文档、回消息。第一次用先在小事上试，确认它只干了你吩咐的，再放开更多权限。
